...
@if (count($users) > 0)
    @foreach ($users as $user)
        <tbody>
            <tr align="center">
                <td><input type="checkbox" id="child{{ $user->id }}" class="sub_chk" data-id={{ $user->id }}>
                </td>
                <td><img class="myImages" id="myImg"
                        src="@if (!empty($user->image->userimage)) {{ asset('public/image/' . $user->image->userimage) }}@else{{ asset('public/blank.png') }} @endif "
                        width="50"></td>
                <td><img class="myImages" id="myImg"
                        src="@if (!empty($user->image->usersign)) {{ asset('public/sign/' . $user->image->usersign) }}@else{{ asset('public/blank.png') }} @endif "
                        width="50"></td>
                ...
                <td>
                    ...
                    <div>
                        <button class="btn btn-info" onclick="editfun(this)" id="edit_age"><i
                                class="far fa-edit"></i></button>
                        <button class="btn btn-success" onclick="savefun(this,{{ $user->id }})"
                            style="display: none;" id="save_age">save</button>
                    </div>
                </td>
                <td>
...
@else
    <tr>
        <td style="font-size: 25px;" colspan="11" align="center"><b>No Data Found</b></td>
    </tr>
@endif
